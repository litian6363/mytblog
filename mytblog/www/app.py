...
# 每次处理url之前都提取解析cookie并绑定在request对象上
...
# 将处理url函数结果都转化为`web.response`对象
...
async def init(loops):
    await orm.create_pool(loop=loops, **configs['db'])
    app = web.Application(loop=loops, middlewares=[logger_factory, auth_factory, response_factory])
    init_jinja2(app, filters=dict(datetime=datetime_filter))
    add_routes(app, 'handlers')
    add_static(app)
    srv = await loops.create_server(app.make_handler(), '127.0.0.1', 9999)
    logging.info('server tt started at http://127.0.0.1:9999...')
    return srv
